...
void Detector::initOpenCV() {
    ...
    CFStringFindAndReplace(urlWithLocalhost, CFSTR("file://"), CFSTR(""),(CFRange) { 0, CFStringGetLength(urlWithLocalhost)}, 0);
...
void Detector::detectAndDisplay() {
    ...
    displayFPS();
    if(personList.size() > 0)
        cout << "Currently: " << personList.size() << " people tracked (" << faces.size() << "), ID: " << personList.begin()->id << ", x: " << personList.begin()->rectangle.x << ", y: " << personList.begin()->rectangle.y << "\n";
